...
//		for (int lineIndex=0; lineIndex < as.lineCount(); lineIndex++)
		IntStream.range(0, as.lineCount()).parallel().forEach((lineIndex) ->
		{
			...
			lineInfo.getLineResults().setPedVerLinesResult(lineStat);
	...
	private void moveParentsToTop()
	{
		...
		p1.getLineResults().getPedVerLinesResult().setP1(true);
		p2.getLineResults().getPedVerLinesResult().setP2(true);
...
// - if line.allele2 matches any allele in P1; score += 05
	...
	private void changeColourScheme()
	{
		...
		viewSet.setColorScheme(ColorScheme.SIMILARITY_TO_EITHER_PARENT);
