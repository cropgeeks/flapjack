...
public class IFBPanel extends JPanel implements ActionListener, ListSelectionListener, ITableViewListener, TableModelListener
{
	public JTabbedPane tabs;

	private LineDataTable table;
	private IFBTableModel model;
	private GTViewSet viewSet;

	private LinkedTableHandler tableHandler;

	private IFBPanelNB controls;
	private IFBSummaryPanelNB summaryControls;

	private int rank = 1;

	public IFBPanel(GTViewSet viewSet)
	{
		controls = new IFBPanelNB(this);
		summaryControls = new IFBSummaryPanelNB(this, viewSet);
		...
		add(new TitlePanel(RB.getString("gui.ifb.IFBPanel.title")), BorderLayout.NORTH);
		...
		summaryControls.autoResize.setSelected(viewSet.getIFBBatchList().isAutoResize());
	}

	private void updateModel(GTViewSet viewset)
	{
		model = new IFBTableModel(viewset);
